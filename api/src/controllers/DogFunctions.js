...
async function getDogs (req, res, next){
    ...
    try {
        ...
        // console.log(allDogs)
        if(name){
            const filterInfo = allDogs.filter(el => el.name.toLowerCase().includes(name.toLowerCase()))
            // console.log(filterInfo)
            filterInfo.length ? res.status(200).json(filterInfo) : res.status(404).send('No dogs found')
        } else {
...
function getDogById (req, res, next){
    ...
    allInfo().then(async (resp) => {
        if(!id.includes('-')){
            ...
            gameFromDb ? res.status(200).json(gameFromDb) : res.status(404).send('Breed from DB not found')
        }
    })
    .catch((err) => next(err))
};
    ...
    try {
        const [dog, created] = await Dog.findOrCreate({
          ...
          defaults: {
            ...
            weightMax,
            life_span,  
            image: "https://www.pngitem.com/pimgs/m/76-767784_clip-art-boxer-dog-silhouette-vector-boxer-dog.png"
          },
        });
        // console.log(dog.toJSON())
        let newDogTemperaments = await Temperament.findAll({ where: { name: temperament }})
        // console.log(newDogTemperaments[0].dataValues)
        let noTemp = await Temperament.findOne({ where: { name: 'No temperaments' }})
        dog.addTemperaments(newDogTemperaments.length ? newDogTemperaments : noTemp);
        res.status(200).send(created === false ? `${name} already exist` : 'Dog created successfully');
    }catch(err){
        next(err)
    }
};
