...
const useForm = () => {
             ...
             function fixed(){
               setInput(function (prevState) {
                //    console.log(prevState.temperament)
                 return {
                   temperament: prevState.temperament === 'No temperaments' ? prevState.temperament : [...input.temperament]
                 };
               });
             };
             fixed()
    ...
    const handleSubmit = (e) => {
        if (input.weightMin && input.weightMax && input.heightMin && input.heightMax && input.life_span) {
            if (!dogsDB.map(el => el.name.toLowerCase()).includes(input.name.toLowerCase().trim())){
                e.preventDefault();
